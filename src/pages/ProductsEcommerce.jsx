...
import { Search, Filter, Star, Heart, ShoppingCart, Leaf, Award, Plus, Minus } from 'lucide-react'
...
const ProductsEcommerce = ({ globalSearchTerm, setGlobalSearchTerm, setCurrentPage }) => {
  ...
  useEffect(() => {
    filterProducts()
  }, [products, searchTerm, selectedCategory, selectedBrand, priceRange, ecoRating, sortBy])
  ...
  const fetchCategories = async () => {
    try {
      ...
      setCategories(['all', ...new Set(ecoProducts.map(product => product.category))])
    }
  }
  ...
  const fetchBrands = async () => {
    try {
      ...
      setBrands(['all', ...new Set(ecoProducts.map(product => product.brand))])
    }
  }
